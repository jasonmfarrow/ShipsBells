...
int hour_bells_from = 8;
// int hour_bells_f = 8;
int hour_bells_to = 22;
// int hour_bells_t = 22;
...
static void update_time() {
  ...
  if (hournow >= hour_bells_from && hournow < hour_bells_to) {
...
static void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  ...
  // Create or update the battery layer every minute
...
static void main_window_load(Window *window) {
  ...
  // Select the background picture (bitmap)
  if(black_on_white) {
  ...
  s_half_layer = text_layer_create(GRect(0,145,71,23));
  s_quarter_layer = text_layer_create(GRect(72,145,72,23));
...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  int hour_bells_from = persist_read_int(HOUR_BELLS_FROM); 
  int hour_bells_to = persist_read_int(HOUR_BELLS_TO); 
  bool black_on_white = persist_read_bool(BLACK_ON_WHITE);
  // Read first item
  Tuple *t = dict_read_first(iterator);
  // For all items
  while(t != NULL) {
    // APP_LOG(APP_LOG_LEVEL_INFO, "Returned Key Value: %s", t->value->cstring);
    ...
    case HOUR_BELLS_FROM:
      // APP_LOG(APP_LOG_LEVEL_INFO, "Hour Bells From key value: %s", t->value->cstring);
      hour_bells_from = atol(t->value->cstring);
      persist_write_int(HOUR_BELLS_FROM, hour_bells_from);
      // APP_LOG(APP_LOG_LEVEL_INFO, "Hour Bells From var value: %d", hour_bells_from);
      break;
    case HOUR_BELLS_TO:
      // APP_LOG(APP_LOG_LEVEL_INFO, "Hour Bells To key value: %s", t->value->cstring);
      hour_bells_to = atol(t->value->cstring);
      persist_write_int(HOUR_BELLS_TO, hour_bells_to);
      // APP_LOG(APP_LOG_LEVEL_INFO, "Hour Bells to var value: %d", hour_bells_to);
      break;  
    default:
  ...
  // APP_LOG(APP_LOG_LEVEL_INFO, "Bells From: %d", hour_bells_from);
  // APP_LOG(APP_LOG_LEVEL_INFO, "Bells To: %d", hour_bells_to);
  // APP_LOG(APP_LOG_LEVEL_INFO, "Bells F: %d", hour_bells_f);
  // APP_LOG(APP_LOG_LEVEL_INFO, "Bells T: %d", hour_bells_t);
  if(half_hour_bells) {
    snprintf(half_buffer, sizeof(half_buffer), "H:On %d", hour_bells_from);
  } else {
    snprintf(half_buffer, sizeof(half_buffer), "H:Off %d", hour_bells_from);
  }
  if(quarter_hour_bells) {
    snprintf(qtr_buffer, sizeof(qtr_buffer), "Q:On %d", hour_bells_to);
  } else {
    snprintf(qtr_buffer, sizeof(qtr_buffer), "Q:Off %d", hour_bells_to);
  }
